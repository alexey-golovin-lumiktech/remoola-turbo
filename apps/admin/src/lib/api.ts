...
export async function request<T>(path: string, init?: RequestOptions): Promise<T | null> {
  ...
  if (method === `GET`) {
    ...
    if (memHit && memHit.expiresAt > now) {
      const staleAfter =
        typeof init?.cache === `object` ? init.cache.staleAfter ?? 10_000 : 10_000;
  ...
  if (method === `GET` && init?.cache && result !== null) {
    const ttl =
      typeof init.cache === `object` ? init.cache.ttl ?? 30_000 : 30_000;
  ...
  try {
    ...
    if (result) {
      ...
      if (process.env.NODE_ENV === `development`)
        console.debug(`[SWR refresh] ${path}`);
    }
  } catch {
...
const search = (path: string, input?: string) =>
  input ? `${path}?search=${encodeURIComponent(input)}` : path;
...
export const api = {
  ...
  contractors: {
    ...
    create: <T, B>(body: B) =>
      request<T>(join(`admin`, `contractors`), { method: `POST`, body: JSON.stringify(body) }),
    patch: <T, B>(id: string, body: B) =>
      request<T>(join(`admin`, `contractors`, id), { method: `PATCH`, body: JSON.stringify(body) }),
    delete: <T>(id: string) =>
      request<T>(join(`admin`, `contractors`, id), { method: `DELETE` }),
  },
  globalSearch: {
    search: <T>(input?: string, init?: Pick<RequestOptions, `signal` | `cache`>) =>
  ...
  contracts: {
    ...
    delete: <T>(id: string) =>
      request<T>(join(`admin`, `contracts`, id), { method: `DELETE` }),
  },
  documents: {
    search: <T>(input?: string, init?: Pick<RequestOptions, `signal` | `cache`>) =>
      request<T>(search(join(`admin`, `documents`), input), { ...init, cache: true }),
    delete: <T>(id: string) =>
      request<T>(join(`admin`, `documents`, id), { method: `DELETE` }),
  },
  payments: {
    list: <T>(init?: Pick<RequestOptions, `signal` | `cache`>) =>
      request<T>(join(`admin`, `payments`), { ...init, cache: true }),
    delete: <T>(id: string) =>
      request<T>(join(`admin`, `payments`, id), { method: `DELETE` }),
  },
  users: {
