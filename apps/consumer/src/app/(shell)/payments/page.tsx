...
export default function PaymentsPage(){
  ...
  const load = () => getJson<Payment[]>(`/payments`).then(setRows).catch(()=>{});
  useEffect(()=>void load(), []);
  ...
  return <>
    ...
    <section className="mt-5">
      <Card actions={<button className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-blue-700"
        onClick={async()=>{
          const contracts = await getJson<Payment[]>(`/contracts`);
          const contractId = contracts?.[0]?.id;
          if (contractId) { await postJson(`/payments`, { contractId, amountCents: 160000 }); load(); }
        }}>Start Payment</button>}>
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead><tr className="text-gray-500"><th className="py-2 pr-4">Payment ID</th><th className="py-2 pr-4">Contract</th><th className="py-2 pr-4">Amount</th><th className="py-2 pr-4">Method</th><th className="py-2 pr-4">Status</th><th className="py-2">Date</th></tr></thead>
            <tbody>
              {rows.map(p=>(
