...
export default function DocumentsPage(){
  ...
  const load = () => getJson<Doc[]>(`/documents`).then(setRows).catch(()=>{});
  useEffect(()=>void load(), []);
  ...
  return <>
    ...
    <section className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
      <Card className="lg:col-span-2" actions={
        <label className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow cursor-pointer">
          Upload<input type="file" className="hidden" onChange={e=>{ const f=e.target.files?.[0]; if (f) upload(f); }} />
        </label>
      }>
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead><tr className="text-gray-500"><th className="py-2 pr-4">Name</th><th className="py-2 pr-4">Type</th><th className="py-2 pr-4">Size</th><th className="py-2">Updated</th></tr></thead>
            <tbody>
              {rows.map(d=>(
      ...
      <Card title="Filters">
        <div className="space-y-3 text-sm">
          <label className="flex items-center gap-2"><input type="checkbox" className="h-4 w-4"/> Invoices</label>
          <label className="flex items-center gap-2"><input type="checkbox" className="h-4 w-4"/> Contracts</label>
          <label className="flex items-center gap-2"><input type="checkbox" className="h-4 w-4"/> Compliance</label>
          <div className="pt-2"><input placeholder="Search docs..." className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500"/></div>
        </div>
      </Card>
    </section>
  </>;
