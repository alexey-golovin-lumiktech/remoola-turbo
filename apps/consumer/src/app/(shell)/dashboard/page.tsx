...
export default function DashboardPage(){
  ...
  const load = () => getJson<DashboardDto>(`/dashboard`).then(setData).catch(()=>{});
  useEffect(()=>void load(),[]);
      ...
      <section className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1"><p className="font-medium text-gray-900">Create Contract</p><p className="mt-1 text-sm text-gray-600">Spin up an MSA + SoW in minutes.</p></div>
            <a href="/contracts" className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-blue-700">Create</a>
          </div>
        </Card>
        <Card>
          <div className="flex items-start gap-3">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-green-50 text-green-600">✅</span>
            <div className="flex-1"><p className="font-medium text-gray-900">Start Payment</p><p className="mt-1 text-sm text-gray-600">On-contract payment flow.</p></div>
            <a href="/payments" className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-blue-700">Pay</a>
          </div>
        </Card>
      </section>
        ...
        <Card title="Open Contracts">
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead><tr className="text-gray-500"><th className="py-2 pr-4">Name</th><th className="py-2 pr-4">Rate</th><th className="py-2 pr-4">Status</th><th className="py-2">Last activity</th></tr></thead>
              <tbody>
      ...
      <section className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card title="Tasks">
          <div className="space-y-4">
            <div><div className="mb-2 flex items-center justify-between text-sm"><span className="font-medium text-gray-900">Onboarding / Compliance</span><span className="text-gray-500">W-9 pack ready</span></div><ProgressBar value={60}/></div>
            <div className="flex items-center gap-3 text-sm text-gray-700"><input type="checkbox" className="h-4 w-4"/><span>KYC in review</span></div>
          </div>
        </Card>
