...
export const postJson = <T>(p: string, body: unknown) => raw<T>(`/consumer` + p, { method: `POST`, body: JSON.stringify(body) }) as Promise<T>;

export const putJson = <T>(p: string, body: unknown) => raw<T>(`/consumer` + p, { method: `PUT`, body: JSON.stringify(body) }) as Promise<T>;

export const patchJson = <T>(p: string, body: unknown) => raw<T>(`/consumer` + p, { method: `PATCH`, body: JSON.stringify(body) }) as Promise<T>;
